export const translations = {
  en: {
    nav: {
      summary: "Summary",
      expenses: "Expenses",
      income: "Income",
      goals: "Goals",
      reports: "Reports",
      settings: "Settings",
    },
    header: {
      language: "Language",
      currency: "Currency",
      settings: "Settings",
      iconStyle: "Icon Style",
      iconsPhosphor: "Phosphor (Modern Duotone)",
      iconsTabler: "Tabler (Geometric Outline)",
      iconsLucide: "Lucide (Classic Outline)",
    },
    settingsDrawer: {
      title: "Settings & Preferences",
      profile: "Profile",
      guest: "Guest",
      signInPrompt: "Sign in to synchronize and protect your finances",
      signIn: "Sign In",
      signOut: "Sign Out",
      navigation: "Quick Navigation",
      theme: "Visual Theme",
      light: "Light",
      dark: "Dark",
      currency: "Main Currency",
      language: "Language",
      iconStyle: "Icon Style",
      systemVersion: "FinTrack v2.0 • Supabase Auth",
      cloudSafe: "Cloud Protected",
      preferences: "Settings & Preferences",
    },
    summary: {
      balance: "Total Balance",
      income: "Income",
      expenses: "Expenses",
      savings: "Savings",
      netSaving: "Net Saving",
      expensesByCategory: "Expenses by Category",
      incomeVsExpenses: "Income vs Expenses",
      recentActivity: "Recent Activity",
      financialInsights: "Financial Insights",
    },
    months: {
      jan: "Jan",
      feb: "Feb",
      mar: "Mar",
      apr: "Apr",
      may: "May",
      jun: "Jun",
      jul: "Jul",
      aug: "Aug",
      sep: "Sep",
      oct: "Oct",
      nov: "Nov",
      dec: "Dec",
    },
    income: {
      title: "Income",
      recurringTitle: "Fixed Incomes",
      newRecurring: "New Fixed Income",
      description: "Recording and analysis of your income.",
      register: "Register Income",
      edit: "Edit",
      totalMonth: "Total Monthly Income",
      mainSource: "Main Source",
      table: {
        date: "Date",
        description: "Description",
        source: "Source",
        amount: "Amount",
      },
      form: {
        title: "Register New Income",
        amount: "Amount",
        source: "Source",
        description: "Description",
        date: "Date",
        paymentMethod: "Payment Method",
        save: "Save Income",
        cancel: "Cancel",
        loading: "Saving...",
        success: "Income registered successfully",
        error: "Error registering income",
      },
      details: {
        title: "Income Details",
        amount: "Amount",
        source: "Source",
        date: "Date",
        paymentMethod: "Payment Method",
        description: "Description",
        id: "Reference ID",
        deleteConfirm: "Are you sure you want to delete this income?",
        deleteSuccess: "Income deleted successfully",
        deleteError: "Error deleting income",
      },
      delete: "Delete",
    },
    expenses: {
      title: "Expenses",
      description: "Recording and analysis of your expenses.",
      register: "Register Expense",
      edit: "Edit",
      totalMonth: "Total Monthly Expense",
      highestCategory: "Highest Category",
      table: {
        date: "Date",
        description: "Description",
        category: "Category",
        amount: "Amount",
      },
      form: {
        title: "Register New Expense",
        amount: "Amount",
        category: "Category",
        description: "Description",
        date: "Date",
        paymentMethod: "Payment Method",
        save: "Save Expense",
        cancel: "Cancel",
        loading: "Saving...",
        success: "Expense registered successfully",
        error: "Error registering expense",
      },
      details: {
        title: "Expense Details",
        amount: "Amount",
        category: "Category",
        date: "Date",
        paymentMethod: "Payment Method",
        description: "Description",
        id: "Reference ID",
        deleteConfirm: "Are you sure you want to delete this expense?",
        deleteSuccess: "Expense deleted successfully",
        deleteError: "Error deleting expense",
      },
      delete: "Delete",
    },
    common: {
      transactions: "transactions",
      noData: "No data for this period",
      loading: "Loading your finances...",
      errorTitle: "Something went wrong",
      errorMessage: "Could not load your financial data right now. Please try again.",
      retry: "Retry",
      summaryDescription: "Visualize your financial health at a glance.",
      allTime: "All Time",
      noRecentActivity: "No recent activity",
      recent: "recent",
    },
    goals: {
      title: "Financial Goals",
      description: "Track your objectives and save for what matters.",
      register: "New Goal",
      name: "Goal Name",
      target: "Target Amount",
      current: "Current Amount",
      deadline: "Deadline",
      category: "Category",
      achieved: "Achieved",
      remaining: "Remaining",
      emptyState: "You don't have any goals yet. Start saving for your dreams!",
      form: {
        success: "Goal registered successfully",
        error: "Error registering goal",
      },
      contribute: "Contribute",
      contributionAmount: "Amount to Contribute",
      contributeSuccess: "Contribution added successfully",
      contributeError: "Error adding contribution",
      contributionToGoal: "Contribution to goal",
      delete: "Delete Goal",
      details: {
        deleteConfirm: "Are you sure you want to delete this goal?",
        deleteSuccess: "Goal deleted successfully",
        deleteError: "Error deleting goal",
      },
    },
    categories: {
      Alimentación: "Food",
      Transporte: "Transport",
      Servicios: "Utilities",
      Entretenimiento: "Entertainment",
      Salud: "Health",
      Metas: "Goals",
      Otros: "Others",
      Ahorro: "Savings",
    },
    sources: {
      Salario: "Salary",
      Freelance: "Freelance",
      Inversiones: "Investments",
      Regalo: "Gift",
      Otros: "Others",
    },
    recurring: {
      expensesTitle: "Fixed & Recurring Expenses",
      expensesSubtitle: "Automatically recorded into your balance on their due date",
      incomesTitle: "Fixed & Recurring Income",
      incomesSubtitle: "Automatically recorded into your balance on payment day",
      sync: "Sync",
      newExpense: "New Fixed Expense",
      newIncome: "New Fixed Income",
      estimatedMonthly: "Estimated Monthly Total",
      activeExpensesDesc: "In active recurring expenses",
      activeIncomesDesc: "In active recurring income",
      scheduledExpenses: "Scheduled Expenses",
      activeRatio: "active",
      autoRegisterActive: "Auto-registration enabled",
      nextDue: "Next Due",
      dueOn: "Due:",
      noExpensesTitle: "No recurring expenses scheduled",
      noExpensesDesc: "Set up regular payments (rent, utilities, subscriptions) so they register automatically on time.",
      noIncomesTitle: "No recurring income scheduled",
      noIncomesDesc: "Set up your biweekly salary, pension, or client retainers so they register automatically.",
      createFirstExpense: "Create First Fixed Expense",
      createFirstIncome: "Set Up First Fixed Income",
      registerNow: "Register Now",
      collectNow: "Collect Now",
      alreadyRegistered: "Already Registered",
      details: "Details",
      hide: "Hide",
      category: "Category",
      source: "Source",
      frequency: "Frequency",
      paymentMethod: "Payment Method",
      mode: "Mode",
      autoOnDue: "Auto on due date",
      manual: "Manual",
      editExpense: "Edit expense",
      editIncome: "Edit income",
      delete: "Delete",
      today: "Today!",
      tomorrow: "Tomorrow",
      inDays: "In {days} days",
      overdueBy: "Overdue by {days}d",
    },
  },
  es: {
    nav: {
      summary: "Resumen",
      expenses: "Gastos",
      income: "Ingresos",
      goals: "Metas",
      reports: "Reportes",
      settings: "Ajustes",
    },
    header: {
      language: "Idioma",
      currency: "Moneda",
      settings: "Ajustes",
      iconStyle: "Estilo de Íconos",
      iconsPhosphor: "Phosphor (Duotono Moderno)",
      iconsTabler: "Tabler (Geométrico)",
      iconsLucide: "Lucide (Clásico Outline)",
    },
    settingsDrawer: {
      title: "Ajustes y Configuración",
      profile: "Perfil",
      guest: "Invitado",
      signInPrompt: "Inicia sesión para sincronizar y proteger tus finanzas",
      signIn: "Iniciar Sesión",
      signOut: "Cerrar Sesión",
      navigation: "Navegación Rápida",
      theme: "Tema Visual",
      light: "Claro",
      dark: "Oscuro",
      currency: "Moneda Principal",
      language: "Idioma",
      iconStyle: "Estilo de Íconos",
      systemVersion: "FinTrack v2.0 • Supabase Auth",
      cloudSafe: "Protegido en la Nube",
      preferences: "Ajustes y Preferencias",
    },
    summary: {
      balance: "Saldo Total",
      income: "Ingresos",
      expenses: "Gastos",
      savings: "Ahorros",
      netSaving: "Ahorro Neto",
      expensesByCategory: "Gastos por Categoría",
      incomeVsExpenses: "Ingresos vs Gastos",
      recentActivity: "Actividad Reciente",
      financialInsights: "Consejos Financieros",
    },
    months: {
      jan: "Ene",
      feb: "Feb",
      mar: "Mar",
      apr: "Abr",
      may: "May",
      jun: "Jun",
      jul: "Jul",
      aug: "Ago",
      sep: "Set",
      oct: "Oct",
      nov: "Nov",
      dec: "Dic",
    },
    income: {
      title: "Ingresos",
      recurringTitle: "Ingresos Fijos",
      newRecurring: "Nuevo Ingreso Fijo",
      description: "Registro y análisis de tus ingresos.",
      register: "Registrar Ingreso",
      edit: "Editar",
      totalMonth: "Ingreso Total del Mes",
      mainSource: "Fuente Principal",
      table: {
        date: "Fecha",
        description: "Descripción",
        source: "Fuente",
        amount: "Monto",
      },
      form: {
        title: "Registrar Nuevo Ingreso",
        amount: "Monto",
        source: "Fuente",
        description: "Descripción",
        date: "Fecha",
        paymentMethod: "Método de Pago",
        save: "Guardar",
        cancel: "Cancelar",
        loading: "Guardando...",
        success: "Ingreso registrado con éxito",
        error: "Error al registrar el ingreso",
      },
      details: {
        title: "Detalles del Ingreso",
        amount: "Monto",
        source: "Fuente",
        date: "Fecha",
        paymentMethod: "Método de Pago",
        description: "Descripción",
        id: "ID de Referencia",
        deleteConfirm: "¿Estás seguro de que deseas eliminar este ingreso?",
        deleteSuccess: "Ingreso eliminado con éxito",
        deleteError: "Error al eliminar el ingreso",
      },
      delete: "Eliminar",
    },
    expenses: {
      title: "Gastos",
      description: "Gestión de tus gastos detallados.",
      register: "Registrar Gasto",
      edit: "Editar",
      totalMonth: "Gasto Total del Mes",
      highestCategory: "Categoría más alta",
      table: {
        date: "Fecha",
        description: "Descripción",
        category: "Categoría",
        amount: "Monto",
      },
      form: {
        title: "Registrar Nuevo Gasto",
        amount: "Monto",
        category: "Categoría",
        description: "Descripción",
        date: "Fecha",
        paymentMethod: "Método de Pago",
        save: "Guardar",
        cancel: "Cancelar",
        loading: "Guardando...",
        success: "Gasto registrado con éxito",
        error: "Error al registrar el gasto",
      },
      details: {
        title: "Detalles del Gasto",
        amount: "Monto",
        category: "Categoría",
        date: "Fecha",
        paymentMethod: "Método de Pago",
        description: "Descripción",
        id: "ID de Referencia",
        deleteConfirm: "¿Estás seguro de que deseas eliminar este gasto?",
        deleteSuccess: "Gasto eliminado con éxito",
        deleteError: "Error al eliminar el gasto",
      },
      delete: "Eliminar",
    },
    common: {
      transactions: "transacciones",
      noData: "No hay datos para este período",
      loading: "Cargando tus finanzas...",
      errorTitle: "Algo no salió como esperábamos",
      errorMessage:
        "No fue posible cargar tu información en este momento. Por favor intenta de nuevo.",
      retry: "Reintentar",
      summaryDescription: "Visualiza tu salud financiera de un vistazo.",
      allTime: "Todo el tiempo",
      noRecentActivity: "No hay actividad reciente",
      recent: "recientes",
    },
    goals: {
      title: "Metas Financieras",
      description: "Rastrea tus objetivos y ahorra para lo que importa.",
      register: "Nueva Meta",
      name: "Nombre de la Meta",
      target: "Monto Objetivo",
      current: "Monto Actual",
      deadline: "Fecha Límite",
      category: "Categoría",
      achieved: "Logrado",
      remaining: "Restante",
      emptyState: "Aún no tienes metas. ¡Empieza a ahorrar para tus sueños!",
      form: {
        success: "Meta registrada con éxito",
        error: "Error al registrar la meta",
      },
      contribute: "Aportar",
      contributionAmount: "Monto a Aportar",
      contributeSuccess: "Aporte realizado con éxito",
      contributeError: "Error al realizar el aporte",
      contributionToGoal: "Aporte a meta",
      delete: "Eliminar",
      details: {
        deleteConfirm: "¿Estás seguro de que deseas eliminar esta meta?",
        deleteSuccess: "Meta eliminada con éxito",
        deleteError: "Error al eliminar la meta",
      },
    },
    categories: {
      Alimentación: "Alimentación",
      Transporte: "Transporte",
      Servicios: "Servicios",
      Entretenimiento: "Entretenimiento",
      Salud: "Salud",
      Metas: "Metas",
      Otros: "Otros",
      Ahorro: "Ahorro",
    },
    sources: {
      Salario: "Salario",
      Freelance: "Freelance",
      Inversiones: "Inversiones",
      Regalo: "Regalo",
      Otros: "Otros",
    },
    recurring: {
      expensesTitle: "Gastos Fijos y Recurrentes",
      expensesSubtitle: "Se registran automáticamente en tu historial al llegar su fecha",
      incomesTitle: "Ingresos Fijos y Recurrentes",
      incomesSubtitle: "Se registran automáticamente en tu balance al llegar su fecha de cobro",
      sync: "Sincronizar",
      newExpense: "Nuevo Gasto Fijo",
      newIncome: "Nuevo Ingreso Fijo",
      estimatedMonthly: "Total Mensual Estimado",
      activeExpensesDesc: "En compromisos fijos activos",
      activeIncomesDesc: "En ingresos fijos activos",
      scheduledExpenses: "Gastos Programados",
      activeRatio: "activos",
      autoRegisterActive: "Auto-registro activado",
      nextDue: "Próximo Cobro",
      dueOn: "Cobro:",
      noExpensesTitle: "No tienes gastos fijos programados",
      noExpensesDesc: "Configura tus pagos habituales (renta, servicios, suscripciones) para que se registren solos al llegar la fecha.",
      noIncomesTitle: "No tienes ingresos fijos configurados",
      noIncomesDesc: "Programa tu salario quincenal, pensión o cobros recurrentes de clientes para que se registren automáticamente.",
      createFirstExpense: "Crear Primer Gasto Fijo",
      createFirstIncome: "Configurar mi primer ingreso fijo",
      registerNow: "Registrar Ahora",
      collectNow: "Cobrar Ahora",
      alreadyRegistered: "Ya Registrado",
      details: "Detalles",
      hide: "Ocultar",
      category: "Categoría",
      source: "Fuente",
      frequency: "Frecuencia",
      paymentMethod: "Medio de Pago",
      mode: "Modo de Cobro",
      autoOnDue: "Automático al vencer",
      manual: "Manual",
      editExpense: "Editar gasto",
      editIncome: "Editar ingreso",
      delete: "Eliminar",
      today: "¡Hoy!",
      tomorrow: "Mañana",
      inDays: "En {days} días",
      overdueBy: "Venció hace {days}d",
    },
  },
};

export type TranslationKey = keyof typeof translations.en;
